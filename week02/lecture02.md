...
2. Outline of today
 * we have to get data from storage to viz -> have to worry about how stored on disk & how to read it into python/javascript/ect
 * how can data be stored in files
    * text - csv, ascii, json
    * binary - hdf5, png/gif/etc
    * query - json/sql
 * how data is stored on disk as binary? (or skip this...)
 * we also have to think about what different operations & filtering we can apply to different types of data
 * why do we need to think about this (example: buildings vs. image & introduce buildings)
 * what can we do: relationships & filtering slides & (sub)sampling & binning (histograms), also averaging, summing, etc (see last semester)
