...
class HistogramCustomD3Component extends D3Component { // creates custom comp.
	...
	console.log('Printing out properties -- INITIAL');
	...
	// we have to delete everything that was drawn before
	var svg = this.svg; // grabbing the svg associated with our component tag
	console.log('svg & props in UPDATE');
	console.log(Object.keys(svg));
	console.log(props.country);

	// select all objects in my svg & remove them all
	...
	d3.csv("https://raw.githubusercontent.com/UIUC-iSchool-DataViz/spring2020/master/week12/corg/corgs_per_country_over_time_columns_2020.csv", function(error, data) {
		...
		.style("text-decoration", "underline")  
		.text(props.country);
	}); // closing the d3.csv reader function
